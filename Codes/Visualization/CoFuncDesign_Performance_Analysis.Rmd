...
# This code was prepared by Ravi Nicholas Balasubramanian
...
###########################################
# Load rna binding data 
###########################################
RNABindingData <- read.csv("designed_sequences_rna_binding_results.csv",row.names=1)

rnaBindDataDf <-  data.frame(original = RNABindingData[1:nrow(RNABindingData),'original_seq'],generated = RNABindingData[1:nrow(RNABindingData),'designed_seq'], length = as.numeric(RNABindingData[1:nrow(RNABindingData),'length']),evalModPerformance = as.numeric(RNABindingData[1:nrow(RNABindingData),'eval_model_performance']),genModPerformance = as.numeric(RNABindingData[1:nrow(RNABindingData),'generator_model_performance']),robustness = as.numeric(RNABindingData[1:nrow(RNABindingData),'robustness']))

rnaBindDataDf$similarity <- mapply(sequence_identity, rnaBindDataDf$original, rnaBindDataDf$generated)
...
breaks <- seq(
  floor(min(rnaBindDataDf$length, na.rm = TRUE)),
  ceiling(max(rnaBindDataDf$length, na.rm = TRUE)) + bin_width,  # add one more bin to cover the top end
  by = bin_width
)
rnaBindDataDf$length_bin <- cut(
  rnaBindDataDf$length,
  breaks = breaks,
  include.lowest = TRUE,
  right = FALSE
)
...
Use the code below to generate all official figures.
...
rnaBindingOverLen <- rnaBindDataDf %>%
  group_by(length_bin) %>%
  summarise(
    length_mean = mean(length),  # average of lengths in the bin
    eval = mean(evalModPerformance),
...
rnaBindingOverLen <- rnaBindingOverLen %>%
  mutate(
    Eval_lower = eval - stdEval,
    Eval_upper = eval + stdEval,
    Gen_lower = gen - stdGen,
...
rnaBinding_plot <- ggplot(data = rnaBindingOverLen, aes(x = length_mean)) +
  geom_ribbon(aes(ymin = Eval_lower, ymax = Eval_upper), fill = "blue", alpha = 0.2) +
  geom_ribbon(aes(ymin = Gen_lower, ymax = Gen_upper), fill = "red", alpha = 0.2) +
  geom_ribbon(aes(ymin = Robust_lower, ymax = Robust_upper), fill = "green4", alpha = 0.2) +
  geom_line(aes(y = eval, color = "Eval. Model \n Performance"), size = 1.5) +
  ...
  geom_line(aes(y = robustness, color = "Robustness"), size = 1.5) +
  labs(
    title = "Performance of RNA Binding Models Versus Length (Binned)",
    x = "Mean Length in Bin (# Residues)",
    y = "Scores",
    color = "Legend"
  ) +
...
ggsave("RNABinding_Performance_Binned_plot.png", plot = rnaBinding_plot, width = 30, height = 10, units = "in", dpi = 300)
...
###################################################################
# Histogram of lengths of sequences for rna binding data
###################################################################
lenHistSolubility <- ggplot(data = rnaBindDataDf, aes(x=length)) + 
  geom_histogram(binwidth = 10,fill="#69b3a2", color="#e9ecef", alpha=0.9) +
  labs(title = "Length of sequences for solubility data",
       x = "Length (# Residues)",
       y = "Frequency") +
...
ggsave("lengthHist_RNABinding.png", plot = lenHistSolubility, width = 6, height = 6, units = "in", dpi = 300)
